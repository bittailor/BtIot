...
public class RfDeviceImpl implements RfDevice {
	private final GPIOPin chipEnable;
	...
	public byte status() {
	   return readRegister(OneByteRegister.REGISTER_STATUS);
	}
	...
	public boolean powerUp() {
		return (readSubRegister(OneByteRegister.REGISTER_CONFIG,1,1) & 0x01) == 0x01 ;
	}
	...
	public TransceiverMode transceiverMode() {
		return (readSubRegister(OneByteRegister.REGISTER_CONFIG,1,0)) == 0x01 ? TransceiverMode.RX_MODE : TransceiverMode.TX_MODE;
	}
	...
	public void chipEnable(boolean value) {
		try {
			chipEnable.setValue(value);
		} catch (IOException e) {
	...
	DataRate dataRate() {
		byte low = readSubRegister(OneByteRegister.REGISTER_RF_SETUP, 1, 5);
		byte high = readSubRegister(OneByteRegister.REGISTER_RF_SETUP, 1, 3);
		if(low == 0x01) {
	...
	boolean receivePipeEnabled(RfPipe pipe) {
		...
		return readSubRegister(OneByteRegister.REGISTER_EN_RXADDR, 1, offset) == 0x01 ;
	}
	...
	void receivePipeEnabled(RfPipe pipe, boolean value) {
		...
		writeSubRegister(OneByteRegister.REGISTER_EN_RXADDR, value?0x01:0x00, 1, offset);
	}
	...
	// write
	byte writeRegister(OneByteRegister register, int value) {
		return writeRegister(register, (byte)value);
	}
	
	byte writeRegister(OneByteRegister register, byte value) {
	...
	byte writeSubRegister(OneByteRegister register, int value, int bitSize, int offset) {
		return writeSubRegister(register, (byte)value, (byte)bitSize, (byte)offset);
	}
	
	byte writeSubRegister(OneByteRegister register, byte value, byte bitSize, byte offset) {
		byte mask = calculateMask(bitSize,offset);
	...
	byte writeRegister(FiveByteRegister register, byte[] value) {
		byte cmd = (byte)(Commands.CMD_W_REGISTER.word | (register.address & Masks.MASK_REGISTER_CMD.mask));
	...
	int writeAndRead(java.nio.ByteBuffer src, java.nio.ByteBuffer dst) {
		try {
